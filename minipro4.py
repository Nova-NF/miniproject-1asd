...
class SistemPendaftaranLowongan:
    def __init__(self):
        ...
        self.tambah_pelamar("Nova", "novanrf@gmail.com", "123456789", "2 tahun", ["Python", "JavaScript"], "Web Developer", posisi="awal")
        self.tambah_pelamar("najla nayla", "nnp@yahoo.com", "987654321", "3 tahun", ["HTML", "CSS"], "UI/UX Designer", posisi="awal")
        self.tambah_pelamar("alya rizki", "cici.kecil@gmail.com", "555555555", "1 tahun", ["Java", "C++"], "Software Developer", posisi="awal")
        self.tambah_pelamar("alysa", "alysadwn@gmail.com", "111111111", "4 tahun", ["SQL", "PHP"], "Web Developer", posisi="awal")
        self.tambah_pelamar("divaa", "dv@yahoo.com", "999999999", "2 tahun", ["Python", "Java"], "Software Developer", posisi="awal")
    ...
    def sorting_pelamar(self, key, order):
        ...
        for pelamar in sorted_pelamar:
            self.tambah_pelamar(pelamar['nama'], pelamar['email'], pelamar['telepon'], pelamar['pengalaman'], pelamar['keahlian'], pelamar['lowongan'])

        print("\nData Pelamar setelah sorting:")
        for pelamar in sorted_pelamar:
            print(f"ID: {pelamar['id']}, Nama: {pelamar['nama']}, {key.capitalize()}: {pelamar[key]}")

    def jump_search_id(self, target_id):
        current = self.head
        step = int(len(self) ** 0.5)
        while current and current.id < target_id:
            current = current.next
        if current and current.id == target_id:
            print(f"Pelamar dengan ID {target_id} ditemukan:")
            print(f"ID: {current.id}, Nama: {current.data['nama']}, Email: {current.data['email']}, Telepon: {current.data['telepon']}, Pengalaman: {current.data['pengalaman']}, Keahlian: {current.data['keahlian']}, Lowongan: {current.data['lowongan']}")
        else:
            print(f"Pelamar dengan ID {target_id} tidak ditemukan.")

    def jump_search_nama(self, target_nama):
        current = self.head
        matching_records = []
        while current:
            if current.data['nama'] == target_nama:
                matching_records.append({
                    "id": current.id,
                    "nama": current.data['nama'],
                    "email": current.data['email'],
                    "telepon": current.data['telepon'],
                    "pengalaman": current.data['pengalaman'],
                    "keahlian": current.data['keahlian'],
                    "lowongan": current.data['lowongan']
                })
            current = current.next

        if matching_records:
            print(f"Pelamar dengan nama '{target_nama}' ditemukan:")
            for record in matching_records:
                print(f"ID: {record['id']}, Nama: {record['nama']}, Email: {record['email']}, Telepon: {record['telepon']}, Pengalaman: {record['pengalaman']}, Keahlian: {record['keahlian']}, Lowongan: {record['lowongan']}")
        else:
            print(f"Tidak ada pelamar dengan nama '{target_nama}'.")


    def search_pelamar(self):
        print("Pilihan pencarian:")
        print("1. Berdasarkan ID")
        print("2. Berdasarkan Nama")
        choice = input("Masukkan nomor pilihan pencarian: ")

        if choice == "1":
            target_id = int(input("Masukkan ID pelamar yang ingin dicari: "))
            self.jump_search_id(target_id)
        elif choice == "2":
            target_nama = input("Masukkan nama pelamar yang ingin dicari: ")
            self.jump_search_nama(target_nama)
        else:
            print("Pilihan tidak valid.")
